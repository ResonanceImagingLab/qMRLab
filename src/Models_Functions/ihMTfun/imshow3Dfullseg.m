function  imshow3Dfullseg( Img, disprange, seg)
%Adjusted by Christopher Rowley - relabelled the sagital and coronal
%buttons add the segmentation input to add a boundary line for
%segmentation
...
if verLessThan('matlab', '8')
    ...
    SegSg = flipdim(permute(seg, [3 1 2 4]),1);   % Sagittal view image
    SegCr = flipdim(permute(seg, [3 2 1 4]),1);   % Coronal view image
else
    ImgSg = flip(permute(Img, [3 1 2 4]),1);   % Sagittal view image
    ImgCr = flip(permute(Img, [3 2 1 4]),1);   % Coronal view image
    SegSg = flip(permute(seg, [3 1 2 4]),1);   % Sagittal view image
    SegCr = flip(permute(seg, [3 2 1 4]),1);   % Coronal view image
end
...
% -=< Slice slider callback function >=-
    function SliceSlider (hObj,event, Img)
        ...
        tempx = get(gca,'children'); 
        set(tempx(end),'cdata',squeeze(Img(:,:,S,:))) %
        caxis([Rmin Rmax])
        s1 = findobj('type','line');
        delete(s1)
        hold on 
              SegAdd(seg)
        hold off
