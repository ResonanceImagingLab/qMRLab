...
function MTsat = ihMT_calcMTsatThruLookupTablewithDummyV3(inputMTw_img, b1, T1, mask, M0, echoSpacing, numExcitation, TR, flipAngle,dummyEchoes)
...
for i = 1:length(B1_vector)
    ...
    for j = 1:length(T1_vector)
        ...
        for k = 1:length(MT_vector)
            ...
            SignalMatrix(k) = ihMTrage_sig_eqn_withDummy(echoSpacing, flipAngle, T1_vector(j), TD, numExcitation - dummyEchoes, 1, MT_vector(k), B1_vector(i), 1, dummyEchoes);  
