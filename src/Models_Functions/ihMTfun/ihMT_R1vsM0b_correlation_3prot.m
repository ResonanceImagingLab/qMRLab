...
sat_dual2 = ihMT_calcMTsatThruLookupTablewithDummyV3( dual2, b1, spT1_map, mask1,spApp_mp2, echoSpacing, numExcitation, TR, flipA, DummyEcho);
sat_pos2  = ihMT_calcMTsatThruLookupTablewithDummyV3( pos2, b1, spT1_map, mask1, spApp_mp2, echoSpacing, numExcitation, TR, flipA, DummyEcho);
sat_neg2  = ihMT_calcMTsatThruLookupTablewithDummyV3( neg2, b1, spT1_map, mask1, spApp_mp2, echoSpacing, numExcitation, TR, flipA, DummyEcho);
...
sat_dual3 = ihMT_calcMTsatThruLookupTablewithDummyV3( dual3, b1, spT1_map, mask1,spApp_mp2, echoSpacing, numExcitation, TR, flipA, DummyEcho);
sat_pos3  = ihMT_calcMTsatThruLookupTablewithDummyV3( pos3, b1, spT1_map, mask1, spApp_mp2, echoSpacing, numExcitation, TR, flipA, DummyEcho);
sat_neg3  = ihMT_calcMTsatThruLookupTablewithDummyV3( neg3, b1, spT1_map, mask1, spApp_mp2, echoSpacing, numExcitation, TR, flipA, DummyEcho);
    ...
    for j = axialStart:axialStop % 1:size(sat_dual,2) % for axial slices
        for k =  1:size(sat_dual1,3) % sagital slices  65
            ...
            if mask(i,j,k) > 0 %&& dual_s(i,j,k,3) > 0
                 ...
                 [M0b_8_dual(i,j,k), ~,  ~]  = ihMT_fit_M0b_v2( b1_1*b1(i,j,k), R1_s(i,j,k), sat_dual1(i,j,k), fitValues_D_8);
                 [M0b_8_pos(i,j,k),  ~,  ~]  = ihMT_fit_M0b_v2( b1_1*b1(i,j,k), R1_s(i,j,k), sat_pos1(i,j,k), fitValues_S_8);               
                 [M0b_8_neg(i,j,k),  ~,  ~]  = ihMT_fit_M0b_v2( b1_1*b1(i,j,k), R1_s(i,j,k), sat_neg1(i,j,k), fitValues_S_8);
                 
                 % TF = 200 protocol
                 [M0b_200_dual(i,j,k), ~, ~] = ihMT_fit_M0b_v2( b1_2*b1(i,j,k), R1_s(i,j,k), sat_dual2(i,j,k),fitValues_D_200);
                 [M0b_200_pos(i,j,k),  ~, ~] = ihMT_fit_M0b_v2( b1_2*b1(i,j,k), R1_s(i,j,k), sat_pos2(i,j,k),fitValues_S_200);
                 [M0b_200_neg(i,j,k),  ~, ~] = ihMT_fit_M0b_v2( b1_2*b1(i,j,k), R1_s(i,j,k), sat_neg2(i,j,k),fitValues_S_200);
                 
                 % TF = 80  protocol
                 [M0b_80_dual(i,j,k), ~, ~] = ihMT_fit_M0b_v2( b1_3*b1(i,j,k), R1_s(i,j,k), sat_dual3(i,j,k),fitValues_D_80);
                 [M0b_80_pos(i,j,k),  ~, ~] = ihMT_fit_M0b_v2( b1_3*b1(i,j,k), R1_s(i,j,k), sat_pos3(i,j,k),fitValues_S_80);
                 [M0b_80_neg(i,j,k),  ~, ~] = ihMT_fit_M0b_v2( b1_3*b1(i,j,k), R1_s(i,j,k), sat_neg3(i,j,k),fitValues_S_80);     
...
% Optimized Approach
fitValues_D_8  = ihMT_generate_R1vsM0B_correlation( R1_s, M0b_8_dual, tempMask, fitValues_D_8, fullfile(OutputDir,'figures/R1vsM0b_8_dual.png'), fullfile(OutputDir,'fitValues_D_8.mat'));
fitValues_SP_8 = ihMT_generate_R1vsM0B_correlation( R1_s, M0b_8_pos, tempMask, fitValues_S_8, fullfile(OutputDir,'figures/R1vsM0b_8_pos.png'), fullfile(OutputDir,'fitValues_SP_8.mat'));
fitValues_SN_8 = ihMT_generate_R1vsM0B_correlation( R1_s, M0b_8_neg, tempMask, fitValues_S_8, fullfile(OutputDir,'figures/R1vsM0b_8_neg.png'), fullfile(OutputDir,'fitValues_SN_8.mat'));


% Boosted Approach
fitValues_D_200  = ihMT_generate_R1vsM0B_correlation( R1_s, M0b_200_dual, tempMask, fitValues_D_200, fullfile(OutputDir,'figures/R1vsM0b_200_dual.png'), fullfile(OutputDir,'fitValues_D_200.mat'));
fitValues_SP_200 = ihMT_generate_R1vsM0B_correlation( R1_s, M0b_200_pos, tempMask, fitValues_S_200, fullfile(OutputDir,'figures/R1vsM0b_200_pos.png'), fullfile(OutputDir,'fitValues_SP_200.mat'));
fitValues_SN_200 = ihMT_generate_R1vsM0B_correlation( R1_s, M0b_200_neg, tempMask, fitValues_S_200, fullfile(OutputDir,'figures/R1vsM0b_200_neg.png'), fullfile(OutputDir,'fitValues_SN_200.mat'));

% MTsat Approach
fitValues_D_80  = ihMT_generate_R1vsM0B_correlation( R1_s, M0b_80_dual, tempMask, fitValues_D_80, fullfile(OutputDir,'figures/R1vsM0b_80_dual.png'), fullfile(OutputDir,'fitValues_D_80.mat'));
fitValues_SP_80 = ihMT_generate_R1vsM0B_correlation( R1_s, M0b_80_pos, tempMask, fitValues_S_80, fullfile(OutputDir,'figures/R1vsM0b_80_pos.png'), fullfile(OutputDir,'fitValues_SP_80.mat'));
fitValues_SN_80 = ihMT_generate_R1vsM0B_correlation( R1_s, M0b_80_neg, tempMask, fitValues_S_80, fullfile(OutputDir,'figures/R1vsM0b_80_neg.png'), fullfile(OutputDir,'fitValues_SN_80.mat'));
