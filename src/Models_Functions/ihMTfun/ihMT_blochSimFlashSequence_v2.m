function [outSig, M, time_vect] = ihMT_blochSimFlashSequence_v2(Params, varargin)
...
if Params.MTC
    ...
    if strcmp( Params.freqPattern,'dualAlternate')
        ...
        % Precompute the RF matrix that is time variant in the 3rd dimension        
        for k = 1:PulseDur
            E_rf2(:,:,k) = ihMT_bloch_McConnell_wDipolar(Params, -Params.delta, satPulse.omega(tSat(k))); 
        end
    end
...
for i = 1:loops
        ...
        if Params.boosted
            ...
            for j1 = 1:Params.satTrainPerBoost % repeat Sat pulse train for specified number of times for boost   
                for j = 1:Params.numSatPulse % for each MTsat pulse cycle
                    ...
                    for k = 1:PulseDur
                        if strcmp( Params.freqPattern,'dualAlternate') && (mod(j,2) == 0)
                            [M_t, time_vect(idx)] = ihMT_calcPoolChange(E_rf2(:,:,k), B, I, stepSize, M_t, time_vect(idx-1) );
                        else % single is default;   
                            [M_t, time_vect(idx)] = ihMT_calcPoolChange( E_rf(:,:,k), B, I, stepSize, M_t, time_vect(idx-1) );
                        end
                ...
                if j1 < Params.satTrainPerBoost % don't include the relaxation time after last one.
                    ...
                    M_t = ihMT_XYmag_Spoil(Params, M_t, Params.TD_MT, 0, 0);
    ...
    for j = 1: Params.numExcitation
        ...
        if i > loops-num2avgOver && (j > Params.DummyEcho) 
            ...
            Sig_vec(rep,j - Params.DummyEcho ) = ihMT_transverseMagnetizationMagnitude(M_t);
